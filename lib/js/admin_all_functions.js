...
function addImagesFields(colorId, addImagesFields) {
	...
	if(typeof(bind_upload_inputs) == 'function') {
		bind_upload_inputs();
	}
}

function lookup(search, id_utilisateur, apply_vat, currency, currency_rate, return_mode_for_displayed_values) {
	( function($) {
		...
		if(search.length == 0) {
			...
			$.post("rpc.php", {search: search, id_utilisateur: id_utilisateur, apply_vat: apply_vat, currency: currency, currency_rate: currency_rate, return_mode_for_displayed_values: return_mode_for_displayed_values}, function(data){
				if(data.length >0) {
					$(divtoshow).show();
					$(divtoshow).html(data);
				}
...
function add_products_list_line(id, ref, nom, purchase_prix, quantite, size_options_html, color_options_html, tva_options_html, purchase_prix_ht, prix_cat, prix_cat_ht, str_add_line, mode) {
...
function delete_products_list_line(id, ordered_product_list) {
