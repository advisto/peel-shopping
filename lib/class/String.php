...
// +----------------------------------------------------------------------+
// $Id: String.php 50572 2016-07-07 12:43:52Z sdelaporte $
if (!defined('IN_PEEL')) {
	die();
}
 ...
 * @version $Id: String.php 50572 2016-07-07 12:43:52Z sdelaporte $
 * @access public
 */
class String {
	/**
	...
	public static function cut_with_separator($string, $max_part_length = 40, $separator = ' ')
	{
		$left_string = '';
		$right_string = $string;
		while(String::strlen($right_string)>$max_part_length) {
			// On évite de faire une fonction récursive pour meilleure performance => on fait une boucle
			$left_string .= String::substr($right_string, 0, $max_part_length) . $separator;
			$right_string = String::substr($right_string, $max_part_length);
		}
		...
		if ($ideal_length_with_clean_cut_if_possible > 0 && $length > $ideal_length_with_clean_cut_if_possible) {
			...
			foreach(array('.', '!', '?', ';', ':', ',', ' ', '=', '+', '-', '{', '}', '[', ']', '(', ')', '<', '>', '_', '#', '*') as $this_separator) {
				...
				if ($possible_cut !== false && $possible_cut < $length_limit - String::strlen($ending_if_no_middle_separator)) {
					...
					$length_limit = $possible_cut + String::strlen($ending_if_no_middle_separator);
		...
		foreach($sentences_array as $this_main_sentence_key => $this_main_sentence) {
			...
			foreach($string_array as $this_main_key => $this_main_string) {
				...
				foreach($tab as $key => $this_string) {
					...
					// On met une condition strlen (et non pas String::strlen) pour aller plus rapide
					if (strlen($this_string) > $length_limit && ($force_shorten_if_special_content || (String::strpos($this_string, 'http') === false && String::strpos($this_string, 'quote=') === false && String::strpos($this_string, '[/quote]') === false && String::strpos($this_string, '&#') === false))) {
						if($add_separator_instead_of_cutting) {
							$tab[$key] = String::cut_with_separator($this_string, $length_limit, $separator);
						} else {
							$tab[$key] = String::str_shorten($this_string, $length_limit, $separator, null, null);
						}
					}
				}
	...
	public static function htmlentities ($string, $flags = ENT_COMPAT, $charset = GENERAL_ENCODING, $suppr_endline = false, $encode_only_isolated_amperstands = false, $decode_html_entities_first = false)
	{
		...
		if (String::strpos($string, '&amp;#') !== false && String::strpos($string, '&amp;#') >= String::strlen($string) - 9) {
			if (String::substr($string, String::strlen($string) - 6, 6) == '&amp;#') {
				$string = String::substr($string, 0, String::strlen($string) - 6);
			} elseif (String::substr($string, String::strlen($string) - 7, 6) == '&amp;#') {
				$string = String::substr($string, 0, String::strlen($string) - 7);
			} elseif (String::substr($string, String::strlen($string) - 8, 6) == '&amp;#' && String::strpos(String::substr($string, String::strlen($string) - 8), ';') === false) {
				$string = String::substr($string, 0, String::strlen($string) - 8);
			} elseif (String::substr($string, String::strlen($string) - 9, 6) == '&amp;#' && String::strpos(String::substr($string, String::strlen($string) - 9), ';') === false) {
				$string = String::substr($string, 0, String::strlen($string) - 8);
			}
		}
		return $string;
	...
	public static function getCleanHTML($text, $max_width = null, $allow_form = false, $allow_object = false, $allow_class = false, $additional_config = null, $safe = true, $additional_elements = null, $max_caracters_length = 50000, $max_octets_length = 59000, $max_word_and_url_length = 100)
	{
		...
		$C['parent'] = isset($C['parent'][0]) ? String::strtolower($C['parent']) : 'body';
			...
			foreach(array('width="' => array('"'), 'width = "' => array('"'), "width='" => array("'"), "width = '" => array("'"), 'width:' => array(';', '"', "'"), 'width :' => array(';', '"', "'"), 'position:' => array(';', '"', "'"), 'position :' => array(';', '"', "'"), 'font-size :' => array(';', '"', "'"), 'font-size:' => array(';', '"', "'")) as $begin_item => $end_item_array) {
				...
				while (($begin_pointer = String::strpos($text_clean, $begin_item, $pointer)) !== false) {
					...
					foreach($end_item_array as $end_item) {
						$this_end_pointer = String::strpos($text_clean, $end_item, $begin_pointer + String::strlen($begin_item));
					...
					$item_value = String::substr($text_clean, $begin_pointer + String::strlen($begin_item), $end_pointer - ($begin_pointer + String::strlen($begin_item)));
					...
					if (is_numeric($item_value) && $item_value > 24 && String::substr($begin_item, 0, 4) == 'font') {
						$item_value = 24;
					...
					$new_text_clean .= String::substr($text_clean, $pointer, $begin_pointer - $pointer) . $begin_item . $item_value;
