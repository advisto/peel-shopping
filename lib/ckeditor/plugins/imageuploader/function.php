...
function loadImages() {
    if(file_exists($GLOBALS['useruploadpath'])){

        $filesizefinal = 0;
        $count = 0;

        $dir = $GLOBALS['useruploadpath'];
        ...
        for($i=count($files)-1; $i >= 0; $i--):
            ...
            $image_url = $site.$GLOBALS['useruploadfolder']."/".$image_basename;
            ...
            if($GLOBALS['file_style'] == "block") { ?>
                <div class="fileDiv"
                    ...
                    <p class="fileDescription"><span class="fileMime"><?php echo $image_extension; ?></span> <?php echo $image_filename; ?><?php if($GLOBALS['file_extens'] == "yes"){echo ".$image_extension";} ?></p>
                    <p class="fileTime"><?php echo date ("F d Y H:i", filemtime($image)); ?></p>
                    <p class="fileTime"><?php echo $filesizetemp; ?> KB</p>
                </div>
            <?php } elseif($GLOBALS['file_style'] == "list") { ?>
                <div class="fullWidthFileDiv"
                     onclick="showEditBar('<?php echo $image_url; ?>','<?php echo $image_height; ?>','<?php echo $count; ?>','<?php echo $image_basename; ?>');"
                     ondblclick="showImage('<?php echo $image_url; ?>','<?php echo $image_height; ?>','<?php echo $image_basename; ?>');"
                     data-imgid="<?php echo $count; ?>">
                    <div class="fullWidthimgDiv"><img class="fullWidthfileImg lazy" data-original="<?php echo $image_url; ?>"></div>
                    <p class="fullWidthfileDescription"><?php echo $image_filename; ?><?php if($GLOBALS['file_extens'] == "yes"){echo ".$image_extension";} ?></p>
        ...
        echo '<div id="folderError">'.$alerts9.' <b>'.$GLOBALS['useruploadfolder'].'</b> '.$alerts10;
    }
}

function pathHistory() {
    $latestpathes = array_slice($GLOBALS['foldershistory'], -3);
