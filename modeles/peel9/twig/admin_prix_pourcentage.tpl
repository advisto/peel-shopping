...
// +----------------------------------------------------------------------+
// $Id: admin_prix_pourcentage.tpl 53200 2017-03-20 11:19:46Z sdelaporte $
#}<form class="entryform form-inline" role="form" method="post" action="{{ action|escape('html') }}">
	...
	<div class="col-sm-5 center">
		...
		<script><!--//--><![CDATA[//><!--
			var new_order_line_html = '<tr class="top" id="sortable_[i]"><td><img src="{{ administrer_url }}/images/b_drop.png" alt="{{ STR_DELETE }}" onclick="if(bootbox.confirm(\'{{ STR_ADMIN_PRODUCT_ORDERED_DELETE_CONFIRM|filtre_javascript(true,true,false) }}\', function(result) {if(result) {admin_delete_products_list_line([i], true);} } )) return false;" title="{{ STR_ADMIN_PRODUCT_ORDERED_DELETE }}" style="cursor:pointer" /> <input type="hidden" name="produits[]" value="[id]"></td><td>[ref] [nom]</td></tr>';
		//--><!]]></script>
