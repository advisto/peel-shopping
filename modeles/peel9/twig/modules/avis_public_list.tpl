...
{% if module_avis_verifie %}
	<div class="NETREVIEWS_PRODUCT_REVIEWS" data-product-id="{{ product_id }}" data-product-name="{{ product_name }}" data-product-url="{{ urlprod }}" data-product-img-url="{{ product_url_image }}"></div>
{% else %}
	{% if are_results %}
		{% if module_avis_no_notation is empty and mode == 'avis' %}
	...
	{% for res in results %}
		<div class="td_avis">
		<i>{{ STR_MODULE_AVIS_OPINION_POSTED_BY }} {{ res.pseudo }} {{ STR_BEFORE_TWO_POINTS }}: {% if not module_avis_no_notation %} {% for foo in 1..res.note %}<img src="{{ star_src|escape('html') }}" alt="" />{% endfor %}{% endif %} {{ STR_ON_DATE_SHORT }} {{ res.date }}</i><br />{{ res.avis|html_entity_decode_if_needed|nl2br_if_needed }}
		</div>
			{% if res.edit_allowed %}
				<div>
