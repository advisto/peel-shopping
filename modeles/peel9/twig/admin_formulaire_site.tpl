...
#}<form class="entryform form-inline" role="form" method="post" action="{{ action|escape('html') }}" enctype="multipart/form-data">
	...
	{% endfor %}
		...
		<tr>
			...
			<td><input style="width:100px" maxlength="3" type="number" class="form-control" name="module_autosend_delay" value="{{ module_autosend_delay|str_form_value }}" /> 
			</td>
		</tr>
		<tr>
	...
	{% for m in modules_infos %}
		...
		<tr>
			...
			<td>
				<input type="radio" name="install[{{ m.technical_code }}]" value="1" {% if m.installed %} checked="checked"{% endif %} /> {{ STR_YES }}
				<input type="radio" name="install[{{ m.technical_code }}]" value="0" {% if not m.installed %} checked="checked"{% endif %} /> {{ STR_NO }}
			</td>
		</tr>
			{% endif %}
			{% if m.configuration_variable %}
		<tr>
			<td>{{ STR_ADMIN_ACTIVATE }}{{ STR_BEFORE_TWO_POINTS }}:</td>
			<td>
				<input type="radio" name="{{ m.configuration_variable }}" value="1" {% if m.enabled %} checked="checked"{% endif %} /> {{ STR_YES }}
				<input type="radio" name="{{ m.configuration_variable }}" value="0" {% if not m.enabled %} checked="checked"{% endif %} /> {{ STR_NO }}
			</td>
		</tr>
			{% endif %}
								...
								<table>
									...
									<tr>
										...
										{% for e_code,e_name in emplacement_array %}
											{% set var_name = 'is_' ~ e_code ~ '_off' %}
											<input type="radio" name="module_{{ m.id }}" id="{{ m.id }}_{{ e_code }}" value="{{ e_code }}"{% if m.location == e_code %} checked="checked"{% endif %}{% if m[var_name] %} disabled="disabled"{% endif %} /> <label for="{{ m.id }}_{{ e_code }}">{{ e_name }}</label>
										{% endfor %}
										</td>
									</tr>
									<tr>
