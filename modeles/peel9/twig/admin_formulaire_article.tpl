...
// +----------------------------------------------------------------------+
// $Id: admin_formulaire_article.tpl 53200 2017-03-20 11:19:46Z sdelaporte $
...
<form class="entryform form-inline" role="form" method="post" action="{{ action|escape('html') }}" enctype="multipart/form-data">
	...
	{% endif %}
		...
		{% if articles_on_short_description_layout %}		
		<tr>
			<td class="title_label">{{ STR_ADMIN_ARTICLES_IS_ON_LAYOUT }}{{ STR_BEFORE_TWO_POINTS }}:</td>
			<td>
				<input type="radio" name="on_layout" value="1"{% if on_layout == '1' %}checked="checked"{% endif %} /> {{ STR_YES }}<br />
				<input type="radio" name="on_layout" value="0"{% if on_layout == '0' or not (on_layout)  %} checked="checked"{% endif %} /> {{ STR_NO }}
			</td>
		</tr>
		{% endif %}
		<tr>
		...
		<tr>
			<td class="title_label">{% if image1 is defined and image1.type == 'img' %}{{ STR_IMAGE }}{% else %}{{ STR_FILE }}{% endif %}{{ STR_BEFORE_TWO_POINTS }}:</td>
			<td>
			{% if (image) %}
				{% include "uploaded_file.tpl" with {'f':image1,'STR_DELETE':STR_DELETE_THIS_FILE } %}
			{% else %}
