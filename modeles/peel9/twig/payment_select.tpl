...
// +----------------------------------------------------------------------+
// $Id: payment_select.tpl 55291 2017-11-27 17:13:45Z sdelaporte $
#}<table>
	<tr>
		<td>
...
{% else %}
			<input {% if payment_alert_text %} onclick='{$payment_alert_text}' {% else %} onclick="if(document.getElementById('payment_method')) {ldelim}document.getElementById('payment_method').style.display='none';{rdelim}"  {% endif %} type="radio" name="payment_technical_code" value="{{ technical_code|str_form_value }}"{% if issel %} checked="checked"{% endif %} /> {{ nom|html_entity_decode_if_needed }}
{% endif %}
	{% if (fprix_tarif) %}
			{{ STR_BEFORE_TWO_POINTS }}: + {{ fprix_tarif }}
	{% endif %}
