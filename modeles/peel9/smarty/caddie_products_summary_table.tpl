...
// +----------------------------------------------------------------------+
// $Id: caddie_products_summary_table.tpl 55048 2017-11-06 13:18:01Z sdelaporte $
*}
<div class="col-sm-12">
	  ...
	  <table class="table caddie table-striped table-hover" aria-label="{$STR_TABLE_SUMMARY_CADDIE|str_form_value}">
		<thead>
			<tr>
				...
				{if !empty($display_ligne_caddie_product_stock)}<th scope="col">{$LANG.STR_STOCK}</th>{/if}
				<th scope="col">{$STR_UNIT_PRICE} {$taxes_displayed}</th>
				{if !empty($show_options_column)}<th scope="col">{$STR_OPTION_PRICE}</th>{/if}
		...
		{foreach $products as $p}
			<tr>
				...
				{if !empty($display_ligne_caddie_product_stock)}<td class="lignecaddie_stock_product center">{$p.stock}</td>{/if}
				<td class="lignecaddie_prix_unitaire center">
					...
					{elseif isset($p.prix_promo)}
						{$p.prix_promo}
					{else}
						{if !isset($p.prix_ht_without_ecotax)}
							{$p.prix}
						{else}
					...
					{if $with_form_fields && is_array($p.quantite)}
						...
						{else}
							<div class="input-group">
								<input type="number" class="form-control" name="quantite[{$p.numero_ligne}]" value="{$p.quantite.value|str_form_value}" {if isset($p.quantite.message)} onchange="if(this.value>{$p.quantite.stock_commandable}) {ldelim}this.value='{$p.quantite.stock_commandable}'; bootbox.alert('{$p.quantite.message|filtre_javascript:true:true:true:false}');{rdelim} "{/if} min="0"/>
