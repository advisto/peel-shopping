...
// +----------------------------------------------------------------------+
// $Id: exaprint.tpl 50572 2016-07-07 12:43:52Z sdelaporte $
*}{if isset($results)}
...
<table class="main_table">
	<tr>
		...
		<td class="center menu"><b>{$LANG.STR_DPD_ORDER_STATUT_CARRIER}</b></td>
	...
	{if $is_module_export_ventes_active}
		<td colspan="5" class="title_label" align="left" style="padding-bottom:15px"><a href="{$export_href|escape:'html'}" class="title_label"><img src="{$excel_src|escape:'html'}" align="absmiddle" alt="" />&nbsp;{$LANG.STR_DPD_VENTES_EXPORT_EXCEL}</a></td>
...
<p class="title_label center"><font size="+1" color="green">{$LANG.STR_DPD_ASKED_DELIVERY}{$STR_BEFORE_TWO_POINTS}: {if isset($statut_livraison_name)}{$statut_livraison_name}{else}{$STR_ADMIN_ALL_ORDERS}{/if}</font></p>
<p class="title_label center"><font size="+1" color="green">{$LANG.STR_DPD_ASKED_CARRIER}{$STR_BEFORE_TWO_POINTS}: {if isset($statut_transporteur_name)}{$statut_transporteur_name}{else}{$STR_ADMIN_ALL_ORDERS}{/if}</font></p>
{else}
<p class="title_label center">{$period_text}</p>
