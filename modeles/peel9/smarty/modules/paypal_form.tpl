...
// +----------------------------------------------------------------------+
// $Id: paypal_form.tpl 53676 2017-04-25 14:51:39Z sdelaporte $
*}{if !empty($enable_paypal_iframe)}
<iframe name="hss_iframe" width="570px" height="540px" style="border:none;"></iframe>
{/if}
<form class="entryform form-inline" role="form" id="paypalForm" action="{$url}" method="post" {if !empty($enable_paypal_iframe)}target="hss_iframe" name="form_iframe"{/if}>
	<input type="hidden" name="charset" value="{$charset|str_form_value}">
	{if !empty($enable_paypal_integral_evolution)}
