...
// +----------------------------------------------------------------------+
// $Id: payment_select.tpl 55146 2017-11-13 10:19:34Z sdelaporte $
*}<table>
	<tr>
		<td>
...
{else}
			<input {if !empty($payment_alert_text)} onclick='{$payment_alert_text}' {else} onclick="if(document.getElementById('payment_method')) {ldelim}document.getElementById('payment_method').style.display='none';{rdelim}" {/if} type="radio" name="payment_technical_code" value="{$technical_code|str_form_value}"{if $issel} checked="checked"{/if} /> {$nom|html_entity_decode_if_needed}
{/if}
	{if !empty($fprix_tarif)}
			{$STR_BEFORE_TWO_POINTS}: + {$fprix_tarif}
	{/if}
