...
// +----------------------------------------------------------------------+
// $Id: contact_form.tpl 53676 2017-04-25 14:51:39Z sdelaporte $
				...
				<tr>
					...
					<td>
						<select class="form-control" id="contact_company" name="contact_company" style="">
							{html_options options=$societe_options selected=$societe_options_selected}
						</select>
					</td>
				</tr>
{/if}
...
{/if}
		...
		{/if}
			...
			<div style="text-align:center; margin-top: 10px;">
				{$token}
				<div style="text-align:center; margin-top: 10px; margin-bottom: 10px;"><input type="submit" class="btn btn-primary btn-lg submit-once-only" value="{$STR_SEND|str_form_value}" />{if !empty($ssl_image_src)}<img alt="SSL" src="{$ssl_image_src}" class="image_ssl right" />{/if}</div>
			</div>
