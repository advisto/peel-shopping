...
// +----------------------------------------------------------------------+
// $Id: admin_formulaire_marque.tpl 53676 2017-04-25 14:51:39Z sdelaporte $
...
{foreach $langs as $l}
		...
		<tr>
			<td colspan="2"><input style="width: 100%" type="text" class="form-control" placeholder="{$l.placeholder}" name="nom_{$l.lng}" value="{$l.nom|html_entity_decode_if_needed|str_form_value}" /></td>
		</tr>
		<tr>
	...
	{/if}
		<tr>
			<td colspan="2" class="center"><br /><input class="btn btn-primary" type="submit" value="{$titre_soumet|str_form_value}" />
				{if $mode!='insere' && !empty($STR_ADMIN_SITE_COUNTRY)}
