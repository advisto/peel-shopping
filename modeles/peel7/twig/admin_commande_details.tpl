...
{% endif %}
	...
	<tr>
		...
		<td class="form_commande_detail">
			...
			{% if is_icirelais_module_active %}<div id="tracking_url"></div><br /><a href="javascript:setTracking('{{ module_shipping_icirelais_tracking_url_txt|filtre_javascript(true,true,true) }}','{{ TEXT_COMMENT_TRACKING|filtre_javascript(true,true,true) }}','{{ TEXT_ERROR_TRACKING|filtre_javascript(true,true,true) }}')">{{ TEXT_CREATE_TRACKING }}</a>{% endif %}
		</td>
	</tr>
	</tr>
	{% if (payment_select) %}
...
function add_order_line(id, ref, nom, purchase_prix, quantite, size_options_html, color_options_html, tva_options_html, purchase_prix_ht, prix_cat, prix_cat_ht) {
	...
	jQuery("#suggestions").html("<p class=\"global_success\">{{ STR_ADMIN_COMMANDER_ADD_LINE_TO_ORDER }}</div>");
}
...
function order_line_calculate(id, mode){
	...
	if(isFinite(parseFloat(p_cat)) && (mode!='final' or document.getElementById("p"+id).value!="-") && (mode!='percentage' or document.getElementById("perc"+id).value!='-')){
