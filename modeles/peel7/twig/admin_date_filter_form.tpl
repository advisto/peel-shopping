...
// +----------------------------------------------------------------------+
// $Id: admin_date_filter_form.tpl 39443 2014-01-06 16:44:24Z sdelaporte $
#}<form class="entryform form-inline" role="form" method="get" action="{{ action|escape('html') }}">
	<table class="main_table">
		...
		<tr><td class="title_label center" colspan="2"><p>{{ STR_ADMIN_TODAY_DATE }} {{ date }}</p></td></tr>
		<tr>
		...
		<tr>
			<td colspan="2" class="center"><p>{% if (submit_html) %}{{ submit_html }}{% else %}<input type="submit" name="submit" value="{{ STR_ADMIN_DISPLAY_RESULTS|str_form_value }}" class="btn btn-primary" />{% endif %}</p></td>
		</tr>
	</table>
</form>	