{# Twig
// This file should be in UTF8 without BOM - Accents examples: éèê
// +----------------------------------------------------------------------+
// | Copyright (c) 2004-2013 Advisto SAS, service PEEL - contact@peel.fr  |
// +----------------------------------------------------------------------+
// | This file is part of PEEL Shopping 7.0.0, which is subject to an	  |
// | opensource GPL license: you are allowed to customize the code		  |
// | for your own needs, but must keep your changes under GPL			  |
// | More information: https://www.peel.fr/lire/licence-gpl-70.html		  |
// +----------------------------------------------------------------------+
// | Author: Advisto SAS, RCS 479 205 452, France, https://www.peel.fr/	  |
// +----------------------------------------------------------------------+
// $Id: admin_date_filter_form.tpl 35064 2013-02-08 14:16:40Z gboussin $
#}<form method="get" action="{{ action|escape('html') }}">
	<table class="full_width" class="main_table">
		<tr><td class="entete" colspan="2">{{ form_title }}</td></tr>
		<tr><td class="label center" colspan="2"><p>{{ STR_ADMIN_TODAY_DATE }} {{ date }}</p></td></tr>
		<tr><td class="label right"><b>{{ from_date_txt }}</b>{{ STR_BEFORE_TWO_POINTS }}: <select name="jour1">
			{% for do in days_options %}
				<option value="{{ do.value|str_form_value }}"{% if do.issel %} selected="selected"{% endif %}>{{ do.name }}</option>
			{% endfor %}
			</select>
			<select name="mois1">
			{% for mo in months_options %}
				<option value="{{ mo.value|str_form_value }}"{% if mo.issel %} selected="selected"{% endif %}>{{ mo.name }}</option>
			{% endfor %}
			</select>
			<select name="an1">
			{% for yo in years_options %}
				<option value="{{ yo.value|str_form_value }}"{% if yo.issel %} selected="selected"{% endif %}>{{ yo.name }}</option>
			{% endfor %}
			</select>
		</td>
		<td class="label"><b>{{ until_date_txt }}</b>{{ STR_BEFORE_TWO_POINTS }}: <select name="jour2">
			{% for do in days2_options %}
				<option value="{{ do.value|str_form_value }}"{% if do.issel %} selected="selected"{% endif %}>{{ do.name }}</option>
			{% endfor %}
			</select>
			<select name="mois2">
			{% for mo in months2_options %}
				<option value="{{ mo.value|str_form_value }}"{% if mo.issel %} selected="selected"{% endif %}>{{ mo.name }}</option>
			{% endfor %}
			</select>
			<select name="an2">
			{% for yo in years2_options %}
				<option value="{{ yo.value|str_form_value }}"{% if yo.issel %} selected="selected"{% endif %}>{{ yo.name }}</option>
			{% endfor %}
			</select>
		</td>
		</tr>
		<tr>
			<td colspan="2" class="center label">{{ information_select_html }}</td>
		</tr>
		<tr>
			<td colspan="2" class="center"><p>{% if (submit_html) %}{{ submit_html }}{% else %}<input type="submit" name="submit" value="{{ STR_ADMIN_DISPLAY_RESULTS|str_form_value }}" class="bouton" />{% endif %}</p></td>
		</tr>
	</table>
</form>	