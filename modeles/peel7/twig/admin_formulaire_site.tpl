					...
					<td>
						<div class="edit_module_attribut">
							<table>
								...
								<tr>
									<td><label for="emplacement_{{ m.id }}">{{ STR_ADMIN_PLACE }}{{ STR_BEFORE_TWO_POINTS }}:</label></td>
									<td>
										{foreach $emplacement_array as $e_code => $e_name}
											{% set var_name = 'is_{{ e_code }}_off' %}
											<input type="radio" name="module_{{ m.id }}" id="{{ m.id }}_{{ e_code }}" value="{{ e_code }}"{% if m.location == {{ e_code }} %} checked="checked"{% endif %}{% if m.is_{{ var_name }}_off %} disabled="disabled"{% endif %} /> <label for="{{ m.id }}_{{ e_code }}">{{ e_name }}</label>
										{/foreach}
									</td>
								</tr>
								<tr>
