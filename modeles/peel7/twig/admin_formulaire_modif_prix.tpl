...
// +----------------------------------------------------------------------+
// $Id: admin_formulaire_modif_prix.tpl 35064 2013-02-08 14:16:40Z gboussin $
#}<form method="post" action="{{ action|escape('html') }}">
	...
	<table class="admin_formulaire_modif_prix">
		...
		{{ res.tr_rollover }}
			...
			<td class="center"><input type="text" name="prix[]" size="15" value="{{ res.prix|str_form_value }}" /> {{ site_symbole }} {{ STR_TTC }}</td>
			<td class="center"><input type="text" name="prix_revendeur[]" size="15" value="{{ res.prix_revendeur|str_form_value }}" /> {{ site_symbole }} {{ STR_TTC }}</td>
			<td class="center"><input type="text" name="prix_achat[]" size="15" value="{{ res.prix_achat|str_form_value }}" /> {{ site_symbole }} {{ STR_TTC }}</td>
			<td class="center"><input type="text" name="promotion[]" size="15" value="{{ res.promotion|str_form_value }}" /> %</td>
		</tr>
		{% endfor %}
		{% else %}
			<tr><td colspan="5" align="center">{{ STR_ADMIN_PRIX_NO_PRODUCT_FOUND }}</td></tr>
		{% endif %}
		<tr><td colspan="5" align="center"><br /><br /><input type="submit" value="{{ STR_ADMIN_PRIX_UPDATE|str_form_value }}" class="bouton" /><br /><br /></td></tr>
	</table>
</form>