...
// +----------------------------------------------------------------------+
// $Id: contact_form.tpl 35330 2013-02-16 18:27:13Z gboussin $
...
<div id="contact">
	...
	<div id="contact_form">{% if (success_msg) and (success_msg) %}<div class="global_success">{{ success_msg|nl2br_if_needed }}</div>{% endif %}
		...
		{% endif %}
			...
			<p{% if is_advistofr_module_active %} class="no-display"{% endif %}><span class="form_mandatory">(*) {{ STR_MANDATORY }}</span></p>
		</form>
	</div>
</div>