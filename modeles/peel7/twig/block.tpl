...
// +----------------------------------------------------------------------+
// $Id: block.tpl 55332 2017-12-01 10:44:06Z sdelaporte $
#}{% if is_simplify_mode is empty %}
		...
		{% if (title) %}
			<table class="full_width">
				<tr>
					...
					<td style="width:40px">{% if is_slider_mode %}<a href="#" class="bt_next" title="{{ STR_NEXT_PAGE }}"> <span></span> </a>{% endif %}</td>
				</tr>
			</table>
		{% endif %}
		</div>
...
{% else %}
	<div class="{{ block_class }} {% if extra_class %} col-sm-{{ block_columns_width_sm }} col-md-{{ block_columns_width_md }} footer_col{% endif %}" {{ block_style }}>{{ content }}</div>
{% endif %}