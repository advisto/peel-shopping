...
{% else %}
			<input {% if moneybookers_active %}onclick="document.getElementById('payment_method').style.display='none';"{% endif %} type="radio" name="payment_technical_code" value="{{ technical_code|str_form_value }}"{% if issel %} checked="checked"{% endif %} /> {{ nom|html_entity_decode_if_needed }}
	{% if (fprix_tarif) %}
			{{ STR_BEFORE_TWO_POINTS }}: + {{ fprix_tarif }}
	{% endif %}
	{% if (tarif_percent) %}
