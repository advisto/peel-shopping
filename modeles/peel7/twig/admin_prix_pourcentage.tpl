...
// +----------------------------------------------------------------------+
// $Id: admin_prix_pourcentage.tpl 44077 2015-02-17 10:20:38Z sdelaporte $
#}<form class="entryform form-inline" role="form" method="post" action="{{ action|escape('html') }}">
	...
	<div class="col-sm-5 center">
		...
		<select class="form-control" id="form_categories" class="formulaire1" name="categories[]" multiple="multiple" style="width:100%" size="15" onchange="var select=document.getElementById('form_products'); for (var i in select.options) {select.options[i].selected=''; }">
	...
	<div class="col-sm-5 center">
		...
		<script><!--//--><![CDATA[//><!--
			var new_order_line_html = '<tr class="top" id="line[i]"><td><img src="{{ administrer_url }}/images/b_drop.png" alt="{{ STR_DELETE }}" onclick="if(bootbox.confirm(\'{{ TR_ADMIN_PRODUCT_ORDERED_DELETE_CONFIRM|filtre_javascript(true,true,false) }}\', function(result) {if(result) {delete_products_list_line([i], true);} } )) return false;" title="{{ STR_ADMIN_PRODUCT_ORDERED_DELETE }}" style="cursor:pointer" /> <input type="hidden" name="produits[]" value="[id]"></td><td>[ref] [nom]</td></tr>';
		//--><!]]></script>
	...
	<div class="center" style="padding-top:15px;">
		...
		<input style="width:150px" class="formulaire3" type="text" class="form-control" {% if (percent_prod) %} value="{{ percent_prod|str_form_value }}"{% endif %} name="percent_prod" size="8" />
