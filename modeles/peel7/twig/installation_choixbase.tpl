...
// +----------------------------------------------------------------------+
// $Id: installation_choixbase.tpl 39443 2014-01-06 16:44:24Z sdelaporte $
#}
<form class="entryform form-inline" role="form" action="verifdroits.php" method="post">
	...
	<table>
		...
		<tr>
			<td>
				<label class="radio"><input type="radio" name="choixbase" id="{{ this_database|str_form_value }}" value="{{ this_database|str_form_value }}" {% if this_database == selected_database %} checked="checked"{% endif %} /> {{ this_database }}</label>
			</td>
		</tr>
		{% endfor %}
	</table>
	...
	<p class="center"><input type="submit" value="{{ STR_CONTINUE|str_form_value }}" class="btn btn-primary btn-large" /></p>
</form>