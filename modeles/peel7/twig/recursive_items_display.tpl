...
<li class="{% if it.has_sons %}dropdown-submenu plus{% else %}minus{% endif %}{% if it.is_current %} current{% endif %}{% if (it.technical_code) %} m_item_{{ it.technical_code }}{% endif %}{% if it.class %} {{ it.class }}{% endif %}">
	...
	{% elseif (it.href) %}
		{{ it.indent }}<a href="{{ it.href|escape('html') }}">{% if it.has_sons and location == 'left' %}<span class="menu_categorie_link">{% if (it.nb) %} <span class="nb_item badge pull-right">{{ it.nb }}</span> {% endif %}{{ it.name|str_shorten(max_length) }}</span> <span class="glyphicon glyphicon-chevron-right" title="+"></span>{% else %}{% if (it.nb) %}<span class="nb_item badge pull-right">{{ it.nb }}</span> {% endif %}{{ it.name|str_shorten(max_length) }}{% endif %}</a>
	{% endif %}
</li>
	{% endif %}
