...
// +----------------------------------------------------------------------+
// $Id: contact_form.tpl 37226 2013-06-11 14:53:03Z sdelaporte $
	...
	<div id="contact_form">{if isset($success_msg) AND !empty($success_msg)}<div class="global_success">{$success_msg|nl2br_if_needed}</div>{/if}
		...
		{/if}
				...
				<tr>
					...
					<td class="{$align}">
						<input class="form" type="tel" id="telephone" name="telephone" value="{$telephone_value|str_form_value}" />{$telephone_error}
					</td>
				</tr>
				{if $is_advistofr_module_active}
				<tr>
