...
<form class="entryform form-inline" role="form" action="verifdroits.php" method="post">
	...
	<table>
		...
		<tr>
			<td>
				<label class="radio"><input type="radio" name="choixbase" id="{$this_database|str_form_value}" value="{$this_database|str_form_value}" {if $this_database == $selected_database} checked="checked"{/if} /> {$this_database}</label>
			</td>
		</tr>
		{/foreach}
	</table>
