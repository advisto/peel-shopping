...
// +----------------------------------------------------------------------+
// $Id: admin_formulaire_article.tpl 36927 2013-05-23 16:15:39Z gboussin $
*}{if !$rubrique_options}
...
<form method="post" action="{$action|escape:'html'}" enctype="multipart/form-data">
	...
	<table class="full_width">
		<tr>
			<td class="entete" colspan="2">{if isset($art_href)}{$STR_ADMIN_ARTICLES_FORM_MODIFY} "{$titre}" - <a href="{$art_href|escape:'html'}" onclick="return(window.open(this.href)?false:true);">Voir en ligne</a>{else}{$STR_ADMIN_ARTICLES_FORM_ADD}{/if}</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<input type="number" name="position" value="{$position|html_entity_decode_if_needed|str_form_value}" />
			</td>
		</tr>
		<tr>
