...
// +----------------------------------------------------------------------+
// $Id: critere_stock.tpl 39443 2014-01-06 16:44:24Z sdelaporte $
*}{if $is_form}
	...
	{if !$condensed_display_mode}
		...
		<table>
			<tr>
				...
				<td>
					...
					{foreach $colors as $c}
						<option value="{$c.id|str_form_value}"{if $c.issel} selected="selected"{/if}{if !$c.isavailable} disabled="disabled"{/if}>{$c.name}{if !$c.isavailable} - {$STR_NO_AVAILABLE}{/if}</option>
					{/foreach}
					</select>
				</td>
			</tr>
		...
		<table>
			<tr>
				...
				<td>
					<select class="form-control" id="{$id_select_size}" name="taille" onchange="update_product_price{$save_suffix_id}()">
