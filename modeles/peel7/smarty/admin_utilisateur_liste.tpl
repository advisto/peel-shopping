...
// +----------------------------------------------------------------------+
// $Id: admin_utilisateur_liste.tpl 37904 2013-08-27 21:19:26Z gboussin $
			...
			<td class="input_search">
				<table class="full_width center" cellpadding="2">
					...
					<tr>
						...
						<td>{$STR_ADMIN_LANGUAGE} {if $is_crons_module_active}{$STR_ADMIN_UTILISATEURS_MANDATORY_FOR_MULTIPLE_SEND}{/if}{$STR_BEFORE_TWO_POINTS}:</td>
					...
					<tr>
						...
						<td>
							<select name="list_produit" id="list_produit" style="width:200px">
								...
								<option value="{$o.value|str_form_value}"{if $o.issel} selected="selected"{/if}>{$o.name} (id={$o.id})</option>
								{/foreach}
							</select>
						</td>
						<td>
					...
					<tr>
						<td>
							<tr>
								...
								<td><input name="with_gold_ad" id="with_gold_ad" type="checkbox" value="1"{if $with_gold_ad == '1'} checked="checked"{/if} /></td>
							</tr>
						</td>
					</tr>
					<tr>
...
<form action="{$wwwroot}/modules/webmail/administrer/webmail_send.php" method="post">
	<table id="tablesForm" class="main_table">
		...
		{$res.tr_rollover}
			<td style="width:70px">
				<table>
					<tr>
						...
						<td><a {if !empty($res.etat)} onclick="return confirm('{$STR_ADMIN_UTILISATEURS_DEACTIVATE_USER}');"{/if} title="{$STR_ADMIN_UTILISATEURS_UPDATE_STATUS|str_form_value}" href="{$res.modif_etat_href|escape:'html'}"><img class="change_status" src="{$res.etat_src|escape:'html'}" alt="" /></a></td>
					</tr>
					<tr>
						<td></td>
			...
			<td class="center"><b>{$res.prenom|html_entity_decode_if_needed} {$res.nom_famille|html_entity_decode_if_needed}</b><br />{if $is_not_demo}<a href="{$wwwroot_in_admin}/modules/webmail/administrer/webmail_send.php?id_utilisateur={$res.id_utilisateur}">{$res.email|html_entity_decode_if_needed}</a>{/if}
			{if $is_annonce_module_active}
				<br />{$res.societe|html_entity_decode_if_needed}, {if $res.siret_length > 6}{$STR_NUMBER}{$res.siret|html_entity_decode_if_needed}{else}{if !empty($res.siret)}<br />
				<span style="color:red">{$STR_SIRET} {$res.siret|html_entity_decode_if_needed}</span>{else}<br /><span style="color:red">{$STR_SIRET} : {$STR_NONE}.</span>{/if}{/if}<br />
			...
			<td class="center">
			{if !empty($res.group_nom) AND !empty($res.group_remise)}
