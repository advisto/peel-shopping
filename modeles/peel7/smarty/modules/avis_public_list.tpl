...
// +----------------------------------------------------------------------+
// $Id: avis_public_list.tpl 44077 2015-02-17 10:20:38Z sdelaporte $
*}
...
{if $are_results}
	...
	<div class="td_avis">
		<i>{$STR_MODULE_AVIS_OPINION_POSTED_BY} {$res.pseudo} {for $foo=1 to $res.note}<img src="{$star_src|escape:'html'}" alt="" />{/for} {$STR_ON_DATE_SHORT} {$res.date}</i><br />{$res.avis|html_entity_decode_if_needed|nl2br_if_needed}
	</div>
	{/foreach}
{else}
	{if $type == 'produit'}
