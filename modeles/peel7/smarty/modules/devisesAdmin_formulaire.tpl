...
// +----------------------------------------------------------------------+
// $Id: devisesAdmin_formulaire.tpl 36927 2013-05-23 16:15:39Z gboussin $
*}<form name="entryform" method="post" action="{$action|escape:'html'}">
	...
	<table class="main_table">
		...
		<tr>
			<td colspan="2">{$STR_MODULE_DEVISES_ADMIN_SYMBOL_AT_RIGHT}</td> <td><input type="radio" name="symbole_place" value="1"{if $symbole_place == "1"} checked="checked"{/if} /></td>
		</tr>
		<tr>
			<td colspan="2">{$STR_MODULE_DEVISES_ADMIN_SYMBOL_AT_LEFT}</td> <td><input type="radio" name="symbole_place" value="0"{if $symbole_place == "0"} checked="checked"{/if} /></td>
		</tr>
		<tr>
