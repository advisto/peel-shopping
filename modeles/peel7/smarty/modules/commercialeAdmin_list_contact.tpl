...
// +----------------------------------------------------------------------+
// $Id: commercialeAdmin_list_contact.tpl 39495 2014-01-14 11:08:09Z sdelaporte $
*}<form class="entryform form-inline" role="form" method="get" action="{$action|escape:'html'}">
	<table class="full_width">
		...
		<tr>
			...
			<td>
				<input name="login_to_contact" type="text" class="form-control" value="{if !empty($login_to_contact)}{$login_to_contact|str_form_value}{/if}" title="{$STR_ADMIN_INPUT_SEARCH|str_form_value}" style="width:200px;" />
			</td>
		</tr>
		<tr>
			<th>{$STR_MODULE_COMMERCIAL_ADMIN_NAME_TO_CONTACT}{$STR_BEFORE_TWO_POINTS}:</th>
			<td>
				<input name="nom_to_contact" type="text" class="form-control" value="{if !empty($nom_to_contact)}{$nom_to_contact|str_form_value}{/if}" title="{$STR_ADMIN_INPUT_SEARCH|str_form_value}" style="width:200px;" />
			</td>
		</tr>
		<tr>
			<th>{$STR_MODULE_COMMERCIAL_ADMIN_ACCOUNT_TYPE}{$STR_BEFORE_TWO_POINTS}:</th>
			<td>
				<select class="form-control" name="account_type"  style="width:200px;">
					<option value="">{$STR_ADMIN_ANY}</option>
					{foreach $account_type_options as $o}
					<option value="{$o.value|html_entity_decode_if_needed|str_form_value}"{if $o.issel} selected="selected"{/if}>{$o.name}</option>
					{/foreach}
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				<select class="form-control" name="admin_id" style="width:200px;">
					<option value="">{$STR_ADMIN_ANY}</option>
					{foreach $admin_options as $o}
					<option value="{$o.value|html_entity_decode_if_needed}"{if $o.issel} selected="selected"{/if}>{$o.name}</option>
					{/foreach}
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				<input name="ad_date" type="text" class="form-control datepicker" value="{if !empty($ad_date)}{$ad_date|str_form_value}{/if}" title="{$STR_ADMIN_INPUT_SEARCH|str_form_value}" style="width:200px;" />
			</td>
		</tr>
		<tr>
			<th>{$STR_ADMIN_REASON}{$STR_BEFORE_TWO_POINTS}:</th>
			<td>
				<select class="form-control" name="form_contact_planified_reason" style="width:200px;">
					<option value="">{$STR_ADMIN_ANY}</option>
					<option value="interesting_profile"{if $form_contact_planified_reason == 'interesting_profile'} selected="selected"{/if}>{$STR_ADMIN_UTILISATEURS_FOLLOWED_REASON_INTERESTING_PROFILE}</option>
					<option value="interested_by_product"{if $form_contact_planified_reason == 'interested_by_product'} selected="selected"{/if}>{$STR_ADMIN_UTILISATEURS_FOLLOWED_REASON_INTERESTED_BY_PRODUCT}</option>
					<option value="payment_expected"{if $form_contact_planified_reason == 'payment_expected'} selected="selected"{/if}>{$STR_ADMIN_UTILISATEURS_FOLLOWED_REASON_PAYMENT_EXPECTED}</option>
					<option value="follow_up"{if $form_contact_planified_reason == 'follow_up'} selected="selected"{/if}>{$STR_ADMIN_UTILISATEURS_FOLLOWED_REASON_FOLLOW_UP}</option>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				<select class="form-control" name="form_contact_planified_actif" style="width:200px;">
					<option value="">{$STR_ADMIN_ANY}</option>
					<option value="TRUE"{if $form_contact_planified_actif == 'TRUE'} selected="selected"{/if}>{$STR_ADMIN_TO_DO}</option>
					<option value="FALSE"{if $form_contact_planified_actif == 'FALSE'} selected="selected"{/if}>{$STR_ADMIN_DONE_OR_CANCELED}</option>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				{foreach $results as $res}
					...
					<td class="center">
						...
						<span style="font-weight:bold; {if isset($res.last_date_color)} color:{$res.last_date_color};{/if}">{$res.last_date}</span>
						{else}
