...
// +----------------------------------------------------------------------+
// $Id: attributsAdmin_liste_by_id.tpl 36927 2013-05-23 16:15:39Z gboussin $
*}
<form method="post" name="associe_produit_attribut" action="{$action|escape:'html'}">
	...
	{foreach $results as $res}
		{$res.tr_rollover}
			<td class="label">{if !empty($res.nom)}{$res.nom|html_entity_decode_if_needed}{else}[{$res.id}]{/if}</td>
			<td>
		{if !empty($res.sub_res) || $res.texte_libre || $res.upload}
			{if !empty($res.sub_res)}
					<select name="attribut_id_{$res.id}[]" multiple="multiple" style="width:100%" size="{if ($res.sub_res|@count)<5}{$res.sub_res|@count}{else}5{/if}">
				{foreach $res.sub_res as $sr}
						<option value="{$sr.value|str_form_value}" {if $sr.issel} selected="selected"{/if}>{$sr.desc|html_entity_decode_if_needed}{if $sr.prix>0} - {$STR_MODULE_ATTRIBUTS_ADMIN_LIST_OPTION_OVERCOST} : {$sr.prix} {$STR_TTC}{/if}</option>
				{/foreach}
					</select>
