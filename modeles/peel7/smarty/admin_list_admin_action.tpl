...
// +----------------------------------------------------------------------+
// $Id: admin_list_admin_action.tpl 36232 2013-04-05 13:16:01Z gboussin $
*}<form method="post" action="{$action|escape:'html'}" enctype="multipart/form-data">
	<table class="full_width" cellpadding="2" >
		...
		<tr>
			...
			<td>
				<input name="account" type="text" value="{if !empty($account)}{$account|str_form_value}{/if}" />
			</td>
		</tr>
		<tr>
			...
			<td>
				<select name="date" id="date" onkeyup="display_input2_element(this.id)" onclick="display_input2_element(this.id)">
					<option value="1"{if $date == '1'} selected="selected"{/if}>{$STR_ADMIN_DATE_ON}</option>
					<option value="2"{if $date == '2'} selected="selected"{/if}>{$STR_ADMIN_DATE_STARTING}</option>
					<option value="3"{if $date == '3'} selected="selected"{/if}>{$STR_ADMIN_DATE_BETWEEN_START}</option>
				</select>
		...
		<tr>
			...
			<td>
				<input name="search" maxlength="100" type="text" title="{$STR_ADMIN_INPUT_SEARCH|str_form_value}" value="{if !empty($search)}{$search|str_form_value}{/if}" style="width:200px;" />
				<select name="type">
