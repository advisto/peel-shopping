...
*}<form method="post" action="{$action|escape:'html'}" enctype="multipart/form-data">
		...
		<tr>
			<td colspan="2">
				<table class="full_width">
					<tr>
						<td class="entete">{if $get_mode == "modif"}{$STR_ADMIN_PRODUITS_UPDATE} "{$nom}" - <a href="{$prod_href|escape:'html'}" onclick="return(window.open(this.href)?false:true);">{$STR_ADMIN_SEE_RESULT_IN_REAL}</a>{else}{$STR_ADMIN_PRODUITS_ADD}{/if}</td>
		...
		<tr>
			...
			<td>
				<select name="id_ecotaxe" style="width:500px">
					...
					<option value="{$o.value|str_form_value}"{if $o.issel} selected="selected"{/if}>{$o.code} {$o.nom|str_shorten:50:'':'...'}{$STR_BEFORE_TWO_POINTS}: {$o.prix} {$STR_TTC}</option>
					{/foreach}
				</select>
			</td>
		</tr>
	...
	{foreach $colors as $c}
		<tr>
			<td colspan="2" class="label"><br />{$STR_ADMIN_PRODUITS_FILE_FOR_COLOR} {$c.nom} ({$STR_ADMIN_PRODUITS_DEFAULT_COLOR_IN_FRONT} <input type="radio" name="default_color_id"{if $c.issel} checked="checked"{/if}value="{$c.coul}" />)</td>
		</tr>
		<tr>
			<td colspan="2">{$STR_ADMIN_PRODUITS_FILES_EXPLAIN}</td>
		</tr>
		...
		<tr>
			<td colspan="2">
				<select name="references[]" multiple="multiple" style="width:100%" size="5">
					<option value="">-------------------------------------------</option>			
					{foreach $produits_options as $o}
					<option value="{$o.value|str_form_value}"{if $o.issel} selected="selected"{/if}>{$o.reference} - {$o.name}</option>
					{/foreach}
				</select>
			</td>
		</tr>
	...
	{if $is_attributes_module_active}
		<tr>
			<td colspan="2">
				<div class="global_help">{$STR_ADMIN_PRODUITS_MANAGE_CRITERIA_INTRO}{$STR_BEFORE_TWO_POINTS}: {if $mode == "maj"}<a href="{$produits_attributs_href|escape:'html'}">{$STR_ADMIN_PRODUITS_MANAGE_CRITERIA_LINK}</a>{else}{$STR_ADMIN_PRODUITS_MANAGE_CRITERIA_TEASER} <a href="{$nom_attributs_href|escape:'html'}">{$nom_attributs_href}</a>{/if}</div></td>
		</tr>
	{/if}
		<tr>
			<td colspan="2">
	...
	{if $is_download_module_active}
		...
		<tr>
			<td class="top label">{$STR_ADMIN_PRODUITS_FILE_NAME}{$STR_BEFORE_TWO_POINTS}:</td>
	...
	{if $is_flash_sell_module_active}
		...
		<tr>
			<td class="label">{$STR_ADMIN_PRODUITS_FLASH_PRICE} <b>{$site_symbole} {$ttc_ht}</b>{$STR_BEFORE_TWO_POINTS}:</td>
			<td class="left"><input style="width:150px" type="text" name="prix_flash" value="{$prix_flash|str_form_value}" /></td>
		</tr>
		<tr>
			<td class="label">{$STR_ADMIN_PRODUITS_FLASH_START_DATE}{$STR_BEFORE_TWO_POINTS}:</td>
			<td><input style="width:150px" type="text" name="flash_start" class="datetimepicker" value="{if !empty($flash_start)}{$flash_start|str_form_value}{/if}" /></td>
		</tr>
		<tr>
			<td class="label">{$STR_ADMIN_PRODUITS_FLASH_END_DATE}{$STR_BEFORE_TWO_POINTS}:</td>
			<td><input style="width:150px" type="text" name="flash_end" class="datetimepicker" value="{if !empty($flash_end)}{$flash_end|str_form_value}{/if}" /></td>
		</tr>
		<tr>
			<td class="top label">{$STR_ADMIN_PRODUITS_IS_ON_FLASH}{$STR_BEFORE_TWO_POINTS}:</td>
	...
	{if $is_gifts_module_active}
		...
		<tr id="on_gift_points_tr">
			<td class="label">{$STR_ADMIN_PRODUITS_GIFT_POINTS_NEEDED}{$STR_BEFORE_TWO_POINTS}:</td>
		...
		<tr>
			...
			<td><input type="number" name="points" value="{$points|str_form_value}" style="width:150px" /></td>
		</tr>
	{/if}
		<tr>
