...
// +----------------------------------------------------------------------+
// $Id: list_admin_actions.php 39443 2014-01-06 16:44:24Z sdelaporte $
...
';
	...
	$tpl->assign('account', $frm['account']);
	$tpl->assign('date', $frm['date']);
	...
	if (!empty($results_array)) {
		...
		foreach ($results_array as $actions) {
			...
			if (!empty($actions['data'])) {
				if ($actions['action'] == 'SEND_EMAIL') {
					$template_infos = getTextAndTitleFromEmailTemplateLang(null, null, $actions['data']);
