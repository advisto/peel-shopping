...
// +----------------------------------------------------------------------+
// $Id: tagcloud.php 64741 2020-10-21 13:48:51Z sdelaporte $
//

define('IN_PEEL_ADMIN', true);
include("../../../configuration.inc.php");
necessite_identification();
necessite_priv("admin_white_label,admin_content");
...
if (check_if_module_active('tagcloud')) {
    switch (vb($_REQUEST['mode'])) {
		...
		case "suppr_select" :
			if (!empty($frm['tag_ids'])) {
				foreach($frm['tag_ids'] as $tag_id){
					supprime_recherche($tag_id);
				}
			}
			affiche_liste_recherche($start);
            break;
