...
// +----------------------------------------------------------------------+
// $Id: direaunami.php 50572 2016-07-07 12:43:52Z sdelaporte $
//
if (defined('IN_PEEL')) {
	return;
}
			...
			for ($numitems = 0; $numitems < $items; $numitems++) {
				if ((!empty($fname[$numitems])) && (!empty($femail[$numitems]))) {
					...
					if ($_SESSION['session_form_direaunami_sent'] < 10) {
						...
						send_email($femail[$numitems], '', '', 'direaunami_sent', $custom_template_tags, null, $GLOBALS['support'], false, false, true, $yemail);
