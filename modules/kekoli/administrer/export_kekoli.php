...
// +----------------------------------------------------------------------+
// $Id: export_kekoli.php 44077 2015-02-17 10:20:38Z sdelaporte $
...
while ($C = fetch_assoc($resC)) {
	...
	$resType = query("SELECT nom_".$_SESSION['session_langue']." AS nom
		FROM peel_types
		WHERE id = '" . intval($C['type'])."' AND " . get_filter_site_cond('types') . " ". $sql_cond_types);
	$type = fetch_assoc($resType);

	//On va chercher le code iso du pays
	$resPays = query("SELECT iso
		FROM peel_pays
		WHERE " . get_filter_site_cond('pays') . " AND ".$sql_cond_pays);
