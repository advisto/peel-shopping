...
// +----------------------------------------------------------------------+
// $Id: profil.php 35067 2013-02-08 14:21:55Z gboussin $
//
...
switch (vb($_REQUEST['mode'])) {
	...
	case "insere" :
		foreach ($GLOBALS['lang_codes'] as $lng) {
			if (!empty($_FILES['document_'.$lng]) && !empty($_FILES['document_'.$lng]['name'])) {
				$frm['document_'.$lng] = upload($_FILES['document_'.$lng], false, 'any', $GLOBALS['site_parameters']['image_max_width'], $GLOBALS['site_parameters']['image_max_height']);
			}
		}
		insere_profil($frm);
